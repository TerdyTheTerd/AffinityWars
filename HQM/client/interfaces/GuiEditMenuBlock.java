...
public class GuiEditMenuBlock extends GuiEditMenu {
    ...
    public GuiEditMenuBlock(GuiBase gui, EntityPlayer player, Object obj, int id, Type type, int amount, ItemPrecision precision) {
    ...
    public GuiEditMenuBlock(GuiBase gui, EntityPlayer player, Element element, int id, final Type type, final int amount, ItemPrecision precision) {
        ...
        textBoxes.add(new TextBoxGroup.TextBox(gui, "", 230, 18, false) {
            ...
            protected void textChanged(GuiBase gui) {
                searchItems.clear();
                Thread thread = new Thread(new Search(getText(), GuiEditMenuBlock.this));
                thread.start();
            }
        });
    }
    ...
    protected void save(GuiBase gui) {
        if (type == Type.BAG_ITEM) {
            ...
            GuiQuestBook.selectedQuest.setBlock(selected, id, type, precision, player);
        }
    }
    ...
    public static class Search implements Runnable {
        ...
        private GuiEditMenuBlock menu;
        private List<Element> elements;
        private long startTime;

        public Search(String search, GuiEditMenuBlock menu) {
            this.search = search;
            this.menu = menu;
        ...
        public static void setResult(GuiEditMenuBlock menu, Search search) {
        ...
        public static class SearchEntry {
            ...
            private GuiEditMenuBlock.Element element;

            public SearchEntry(String searchString, String advSearchString, GuiEditMenuBlock.Element element) {
                this.toolTip = searchString;
                this.advToolTip = advSearchString;
                this.element = element;
            }

            public void search(Pattern pattern, List<GuiEditMenuBlock.Element> stackList, boolean advanced) {
    ...
    public static class ThreadingHandler {
        private Map<GuiEditMenuBlock, Search> handle = new LinkedHashMap<>();
        ...
        public void renderEvent(RenderWorldLastEvent e) {
            if (!handle.isEmpty()) {
                for (Map.Entry<GuiEditMenuBlock, Search> entry : handle.entrySet()) {
        ...
        private static void handle(GuiEditMenuBlock menu, Search search) {

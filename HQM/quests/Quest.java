...
public class Quest {
    ...
    public void setBlock(GuiEditMenuBlock.Element element, int id, GuiEditMenuBlock.Type type, ItemPrecision precision, EntityPlayer player) {
        if (type == GuiEditMenuBlock.Type.REWARD || type == GuiEditMenuBlock.Type.PICK_REWARD) {
            if (element instanceof GuiEditMenuBlock.ElementItem) {
                ItemStack itemStack = ((GuiEditMenuBlock.ElementItem) element).getItem();
                if (itemStack != null) {
                    itemStack.stackSize = Math.min(127, element.getAmount());
                    setReward(itemStack, id, type == GuiEditMenuBlock.Type.REWARD);
                }
            }
        } else if (selectedTask != null && selectedTask instanceof QuestTaskItems) {
            ((QuestTaskItems) selectedTask).setBlock(element, id, precision);
        } else if (selectedTask != null && selectedTask instanceof QuestTaskLocation && type == GuiEditMenuBlock.Type.LOCATION) {
            ((QuestTaskLocation) selectedTask).setIcon(id, (ItemStack) element.getItem(), player);
        } else if (selectedTask != null && selectedTask instanceof QuestTaskMob && type == GuiEditMenuBlock.Type.MOB) {
            ((QuestTaskMob) selectedTask).setIcon(id, (ItemStack) element.getItem(), player);
        }
    }
    ...
    public enum TaskType {
    	...
    	BREAK(QuestTaskBreak.class, "break");

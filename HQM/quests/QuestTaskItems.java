...
import hardcorequesting.client.interfaces.GuiEditMenuBlock;
...
public abstract class QuestTaskItems extends QuestTask {
    ...
    public void setBlock(GuiEditMenuBlock.Element element, int id, ItemPrecision precision) {
        if (element.getItem() == null) return;
        ...
        if (id < items.length) {
            if (element instanceof GuiEditMenuBlock.ElementItem) {
                GuiEditMenuBlock.ElementItem item = (GuiEditMenuBlock.ElementItem) element;
                items[id].hasItem = true;
                items[id].fluid = null;
                ItemStack itemStack = item.getItem().copy();
                itemStack.stackSize = 1;
                ...
                GuiEditMenuBlock.ElementFluid fluid = (GuiEditMenuBlock.ElementFluid) element;
                items[id].hasItem = false;
                items[id].fluid = fluid.getItem();
                items[id].item = null;
            }
